...
import { getEmpresaActiva, setEmpresaActiva } from '@/shared/empresaActiva';

const NAV_OPEN_KEY = 'dercas-nav-open';
...
function readOpenSections() {
  ...
  return Object.fromEntries(NAV_SECTIONS.map((section) => [section.id, section.defaultOpen !== false]));
}
...
export function AppLayout() {
  ...
  const [empresaId, setEmpresaIdState] = useState(() => {
    const stored = getEmpresaActiva();
    if (stored) {
      return stored;
    }
    return usuario?.idEmpresa != null ? String(usuario.idEmpresa) : '';
  });
    ...
    async function loadEmpresas() {
      try {
        ...
        setEmpresaIdState((current) => {
          if (current && (current === '' || activas.some((item) => String(item.id) === current))) {
            return current;
          }

          if (usuario?.idEmpresa != null) {
            const next = String(usuario.idEmpresa);
            setEmpresaActiva(next);
            return next;
          }

          if (puedeCambiarEmpresa) {
            setEmpresaActiva('');
            return '';
          }

          const next = activas[0] ? String(activas[0].id) : '';
          setEmpresaActiva(next);
          return next;
        });
      } catch {
          ...
          {navSections.map((section) => {
            ...
            return (
              <div key={section.id} className="mt-3">
                <button
                  type="button"
                  className="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400 hover:bg-slate-800 hover:text-slate-200"
                  aria-expanded={isOpen}
                  onClick={() => {
                    setOpenSections((current) => {
                      const next = { ...current, [section.id]: !isOpen };
        ...
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-white">
          <div className="flex items-center gap-4 px-4 py-3 sm:px-6">
            ...
            <button
              ...
              onClick={() => navigate('/activos/escanear')}
            >
            ...
            <div className="min-w-0">
              ...
              >
                {puedeCambiarEmpresa ? <option value="">Todas las empresas</option> : null}
                {empresas.length === 0 && !puedeCambiarEmpresa ? (
