...
public sealed class GetActivoByIdQueryHandler : IQueryHandler<GetActivoByIdQuery, ActivoDto>
{
    ...
    public async Task<ActivoDto> HandleAsync(GetActivoByIdQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var contextos = await ActivoContextoConsulta.CargarAsync(_db, [entity], estados, cancellationToken);
        var estado = estados.GetValueOrDefault(entity.Id)?.Estado ?? ActivoEstadoOperativo.Disponible;
        return ActivoDtoFactory.Create(entity, estado, contextos.GetValueOrDefault(entity.Id));
    }
}
