...
public sealed class AsignacionesController : ApiControllerBase
{
    ...
    {
        var publicAppUrl = Request.Headers.Origin.FirstOrDefault()
            ?? Request.GetTypedHeaders().Referer?.GetLeftPart(UriPartial.Authority)
            ?? "http://localhost:5173";
        var file = await _pdf.HandleAsync(new GetAsignacionPdfQuery(id, publicAppUrl), cancellationToken);
