...
function JornadaForm({ initialValues, sedeOptions, onSubmit, onCancel, isSubmitting }) {
...
function HallazgoForm({ initialValues, activoOptions, onSubmit, onCancel, isSubmitting }) {
  const { values, errors, touched, handleChange, handleBlur, handleSubmit } = useForm({
    initialValues,
    validate: (next) => {
      const result = {};
...
export function InventarioPage() {
  ...
  const activosDeJornada = jornada
    ? activos.filter((item) => ubicacionById.get(item.idUbicacion)?.idSede === jornada.idSede)
    : activos;
  ...
  return (
    <section className="space-y-6">
      <PageHeader
        ...
        description="Jornadas por sede: registra hallazgos y cierra para ver faltantes y sobrantes."
        actions={
          puedeEditar ? (
      ...
      >
        <HallazgoForm
          ...
          activoOptions={activosDeJornada.map((item) => ({ value: item.id, label: item.nombre }))}
